...
export class EnrolleeService {
  ...
  updateEnrollee = async (context: RouterContext) => {
    ...
    } else if (body.hasOwnProperty('dateOfBirth') && !/([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))/.test(body.dateOfBirth)) {
      // this check DOB being passed from client and validates if proper format
      this.buildResponse(context, 400, 'Please supply DOB in correct format: YYYY-MM-DD')
    }else {
      this.validateId(context, id, (id: string) => {
        enrollees[id] = body;
        body.id = id;
